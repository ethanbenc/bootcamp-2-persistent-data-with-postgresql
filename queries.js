    ...
    */
      async function retrieveAllListings() {
          ...
          try {
            ...
            console.log(listings.every(listing => listing instanceof Listing));
            console.log("All entries:", JSON.stringify(listings));
          } catch(error) {
            console.log('Unable to retrieve all listings', error);
          }
      }
    /* 
   ...
   */
    async function findLibraryWest() {
      ...
      try {
        ...
        console.log("Unable to find library west", error);
      }
    }
      ...
      async function removeCable() {
        ...
        try {
          ...
          if (listing) {
            ...
            console.log("Cable BLD not found");
          }
        } catch(error) {
          console.log("Unable to remove Cable BLDG", error);
        }
    }
    ...
    */
    async function addDSIT() {
      ...
      try {
        const listing = await Listing.findOne({ where: { code: 'DSIT', name: "Data Science and IT Building"} } );
        if (listing) {
          console.log("DSIT Building already created");
        }
        else {
          try {
            const dsit = await Listing.create({ code: "DSIT", name: "Data Science and IT Building" });
            console.log("Created DSIT Building", JSON.stringify(dsit));
          } catch (error) {
            console.log("Unable to create DSIT building", error);
          }
          
        }
        
      } catch(error) {
        console.log("Unable to create new building", error);
      }
    }
    ...
    */
    async function updatePhelpsLab() {
       ...
       try {
        const listing = await Listing.findOne({ where: { code: "PHL" , name: "Phelps Laboratory" } });
        if (listing) {
          listing.address = '1953 Museum Rd, Gainesville, FL 32603, United States';
          await listing.save();
          console.log('Updated Phelps Lab', listing.toJSON());
        }
        else {
          console.log('Phelps Lab Not Found');
        }
      } catch(error) {
        console.log("Unable to Update Phelps Lab", error);
      }
    }
